...
use crate::{data_structures::{problem::Problem, solution::Solution}, world::World};
...
impl MyWindow {
    ...
    pub fn run(self, fps: usize) -> thread::JoinHandle<()> {
        thread::spawn(move || {
            self.create_window(fps);
        })
    }

    fn create_window(&self, fps: usize) {
        ...
        while window.is_open() && !window.is_key_down(Key::Escape) {
            ...
            for y in 0..height {
                for x in 0..width { 
                    ...
                    let value_norm = (value - World::LOWER_BOUND as f64) / ((World::UPPER_BOUND - World::LOWER_BOUND) as f64);
